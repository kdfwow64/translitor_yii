...
class AdsController extends Controller
{
    ...
    public function beforeAction($action)
    {
        ...
        if ($seoTag) {
            ...
            if ($action->id != 'view') {
                ...
                if (!$seoTag->title) {
                    Yii::$app->view->title = Yii::$app->params['site_name'];
                    Yii::$app->view->registerMetaTag([
                        'property' => 'og:title',
                        'content' => Yii::$app->params['site_name']
                    ]);
                } else {
    ...
    public function actionIndex()
    {
        ...
        $query = Ads::find()->where(['active' => 1, 'type_ad' => Ads::SALE])->orderBy('updated_at DESC')->with(['adsAttachments', 'cityName', 'countryName']);
        if ($get) {
...
//            'keyword' => '',
    ...
    {
        if ($model = Ads::find()->where(['slug' => $slug])->with(['adsAttachments', 'cityName', 'countryName', 'user', 'user.security'])->one()) {
            $model->updateCounters(['views' => 1]);
            $works_name = Purpose::find()->where(['id' => $model->type_id])->orderBy('title_'.Yii::$app->language)->asArray()->all();
            $works_name = ArrayHelper::map($works_name, 'id', 'title_'.Yii::$app->language);
            $countries = NetCountry::find()->where(['id' => $model->country])->asArray()->orderBy('name_'.Yii::$app->language)->all();
            $countries = ArrayHelper::map($countries, 'id', 'name_'.Yii::$app->language);
            $jobcat = PropertyType::find()->asArray()->where(['id' => $model->cat_id])->all();
            ...
            Yii::$app->view->title = $model->title . ($model->countryName->$lang_pref ? ' | ' . $model->countryName->$lang_pref : '') .
                ...
                ' | ' . $works_name[$model->type_id] . ' | '.Yii::t('app', 'Landlords').' | '.Yii::$app->params['site_name'];
